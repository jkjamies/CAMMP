...
class GenerateUseCaseStep(
    ...
    override suspend fun execute(params: UseCaseParams): StepResult = runStepCatching {
        ...
        val targetPackage = if (basePackage.endsWith(PackageSuffixes.USE_CASE)) {
            basePackage
        } else {
            "$basePackage${PackageSuffixes.USE_CASE}"
        }

        // Calculate base domain package(e.g. com.example.domain)
        val marker = PackageSuffixes.DOMAIN
        ...
        StepResult.Success(message = message)
    }
}